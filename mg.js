...
function krijoBordin() {
    ...
    document.getElementById("timer").textContent = "Koha: 0 sekonda";

    clearInterval(kohaId); 
    kohaId = setInterval(() => {
        sekonda++;
        document.getElementById("timer").textContent = "Koha: " + sekonda + " sekonda";
    }, 1000);
...
function kontrolloKardat() {
    ...
    if (kartatENjejta.length === kartatArray.length) {
        clearInterval(kohaId); 
        alert("Fituat! Koha juaj ishte: " + sekonda + " sekonda");
